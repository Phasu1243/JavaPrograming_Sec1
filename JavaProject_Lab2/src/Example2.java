import java.text.*; //1. import for formal number
public class Example2 {

	public static void main(String[] args) {
		//2.call Class DecimalFormat
		DecimalFormat frm = new DecimalFormat("#,###.00");
		final double BUFFET = 299;
		int numberofCustomer = 5;
		double netPrice = BUFFET * numberofCustomer;
		System.out.println("Buffet of "+numberofCustomer +" customer is "+ netPrice);
		double serviceCharge = netPrice + (netPrice*0.03);
		System.out.println("Add Service Charge 3% is "+serviceCharge);
				     
		

	}

}
